...
test('moves through the representative FinMate mobile flow', async ({ page }) => {
  ...
  await expect(page.getByText('유럽 여행 자금', { exact: true })).toBeVisible()
  await expect(page.getByText('2,000,000 KRW')).toBeVisible()
  await page.getByRole('link', { name: '퀘스트' }).click()
  await page.getByRole('button', { name: '자동저축 입금 반영 확인하기 수락' }).click()
  ...
  await expect(page.getByRole('heading', { name: '오늘의 금융 활동' })).toBeVisible()
