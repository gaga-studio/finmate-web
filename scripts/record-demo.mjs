...
try {
  ...
  await Promise.all([page.waitForURL('**/onboarding/1'), page.getByRole('button', { name: '회원가입' }).click()])
  await addPrototypeLabel()
  ...
  await page.getByRole('button', { name: '곰 소비 리포트 보기' }).first().click()
  await holdUntil(24)
  await page.getByRole('button', { name: '홈으로' }).click()
  await page.getByRole('button', { name: '토끼 투자 판단 리포트 보기' }).first().click()
  await holdUntil(27)
  await page.getByRole('button', { name: '홈으로' }).click()
  await page.getByRole('button', { name: '물개 저축 리포트 보기' }).first().click()
  await holdUntil(30)
  await page.getByRole('button', { name: '홈으로' }).click()

  await page.getByRole('link', { name: '퀘스트', exact: true }).click()
  await holdUntil(31)
  await page.getByRole('link', { name: /퀘스트 상세: 자동저축 입금 반영 확인하기/ }).click()
  await holdUntil(34)
  ...
  await goto('/record')
  await holdUntil(74)
  await page.getByRole('button', { name: '2026-07-11 기록 상세 보기' }).click()
  await holdUntil(78)
