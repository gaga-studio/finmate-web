...
export function QuestOverviewView({ page, pointLedger, cosmetics }: { page: Schema['QuestPage']; pointLedger: Schema['PointLedgerView']; cosmetics: Schema['CosmeticCatalogView'] }) {
  ...
  return <>
    ...
    {tab === 'available' ? <div className="mate-tab-stack">
      <section className="mate-card quest-summary-card"><QuestNumberedHeading n={1} title="퀘스트 요약"/><div className="quest-summary-top"><div className="quest-summary-left"><div className="quest-stat-row"><div className="quest-stat-box"><span>오늘 퀘스트 진행 현황</span><strong>완료 {page.completedTodayCount} / {page.totalTodayCount}개</strong><small>서버에서 확인된 완료 건수예요.</small></div><div className="quest-stat-box"><span>누적 퀘스트 경험치</span><strong className="quest-stat-pills"><span className="quest-pill xp">XP {page.totalXp}</span><span className="quest-pill point">P {pointLedger.balance}</span></strong></div></div></div>{nextCosmetic ? <div className="quest-reward-box"><QuestIcon src={`${QUEST_ASSET_DIR}/quest-icon-chest.png`} tone="teal" size={44}/><span className="quest-reward-copy"><b>다음 꾸미기 해금</b><strong>{nextCosmetic.name}</strong><small>{nextCosmetic.pricePoints.toLocaleString('ko-KR')}P 필요 · 확정형 보상</small></span></div> : null}<div className="quest-summary-mascot"><QuestMascot size={132}/></div></div>{recommended ? <Link aria-label={`추천 퀘스트: ${recommended.title}`} className="quest-ai-box" to={`/quests/${recommended.questId}`}><span className="quest-ai-recommend"><QuestIcon src={iconFor(recommended)} size={40}/><span><small>현재 홈 목표와 연결된 추천 퀘스트</small><strong>{recommended.title}</strong></span></span><span className="quest-ai-message"><span className="quest-ai-tag">TIP</span><span><b>코치 설명</b><p>작은 행동을 먼저 기록하고 금융 성장은 새 데이터가 확인된 뒤 반영해요.</p></span></span></Link> : null}</section>
