...
import { QuestIcon, QuestMascot, QuestNumberedHeading, QUEST_ASSET_DIR } from '../design-v2/QuestShared'
...
export function QuestOverviewView({ page, pointLedger, cosmetics }: { page: Schema['QuestPage']; pointLedger: Schema['PointLedgerView']; cosmetics: Schema['CosmeticCatalogView'] }) {
  ...
  return <>
    ...
    {tab === 'available' ? <div className="mate-tab-stack">
      <section className="mate-card quest-summary-card"><QuestNumberedHeading n={1} title="퀘스트 요약"/><div className="quest-summary-top"><div className="quest-summary-progress"><span className="quest-summary-label">오늘 퀘스트 진행 현황</span><strong className="quest-summary-count">완료 {page.completedTodayCount}<em> / {page.totalTodayCount}개</em></strong><small>서버에서 확인된 완료 건수예요.</small></div><div className="quest-summary-mascot"><QuestMascot size={112}/></div></div><div className="quest-summary-meta"><div className="quest-stat-box"><span>누적 퀘스트 경험치</span><strong className="quest-stat-pills"><span className="quest-pill xp">XP {page.totalXp}</span><span className="quest-pill point">P {pointLedger.balance}</span></strong></div>{nextCosmetic ? <div className="quest-reward-box"><QuestIcon src={`${QUEST_ASSET_DIR}/quest-icon-chest.png`} tone="teal" size={44}/><span className="quest-reward-copy"><b>다음 꾸미기 해금</b><strong>{nextCosmetic.name}</strong><small>{nextCosmetic.pricePoints.toLocaleString('ko-KR')}P 필요 · 확정형 보상</small></span></div> : null}</div>{recommended ? <Link aria-label={`추천 퀘스트: ${recommended.title}`} className="quest-ai-box" to={`/quests/${recommended.questId}`}><span className="quest-ai-recommend"><QuestIcon src={iconFor(recommended)} size={40}/><span><small>현재 홈 목표와 연결된 추천 퀘스트</small><strong>{recommended.title}</strong></span></span><span className="quest-ai-message"><span className="quest-ai-tag">TIP</span><span><b>코치 설명</b><p>작은 행동을 먼저 기록하고 금융 성장은 새 데이터가 확인된 뒤 반영해요.</p></span></span></Link> : null}</section>
      ...
      <section className="mate-card"><QuestNumberedHeading n={2} title="진행 중·참여 가능" action={<span className="quest-count-pill">{available.length}개</span>}/><div className="quest-progress-list">{available.map((quest) => <Link aria-label={`퀘스트 상세: ${quest.title}`} className="quest-progress-row" to={`/quests/${quest.questId}`} key={quest.questId}><QuestIcon src={iconFor(quest)} size={46}/><span className="quest-progress-body"><strong>{quest.title}</strong><span className="quest-progress-meta"><span>{statusLabel[quest.status]}</span><span>{quest.durationLabel}</span></span><small className="quest-progress-value">현재 {quest.currentValue.toLocaleString('ko-KR')} / 목표 {quest.targetValue.toLocaleString('ko-KR')} {quest.unit === 'KRW' ? '원' : quest.unit === 'BASIS_POINTS' ? 'bp' : '회'}</small></span><span className="quest-progress-reward"><strong>XP {quest.xpReward}</strong><span>꾸미기 {quest.pointReward}P</span><em>금융 스탯 즉시 변경 없음</em></span></Link>)}</div></section>
    </div> : null}
