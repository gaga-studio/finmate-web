...
export function MateDiscovery({ groups }: { groups: MateGroup[] }) {
  ...
  return <div className="mate-tab-stack">
    {primary ? <section className="mate-card mate-group-hero">
      <header className="mate-group-hero-head"><IconBadge icon="profile" tone="teal"/><h2>나와 비슷한 그룹</h2></header>
      <div className="mate-group-trio"><MateAvatar species="otter" size={84} fit="contain"/><MateAvatar species="rabbit" size={84} fit="contain"/><MateAvatar species="bear" size={84} fit="contain"/></div>
      <div className="mate-group-hero-copy"><strong>{primary.name}</strong><span className="mate-group-hero-badge">그룹 구성원 {primary.memberCount.toLocaleString('ko-KR')}명</span><p>비슷한 출발점에서 루틴을 이어온 익명 그룹이에요.</p></div>
      <Link className="app-button primary mate-group-hero-cta" to={`/mates/group/${primary.groupId}`}>그룹 루틴 살펴보기<Chevron/></Link>
    </section> : null}
