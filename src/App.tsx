...
function RecordDaySheet({ date, onClose }: { date: string; onClose: () => void }) {
  ...
  return <div className={styles.scrim} onClick={onClose}><section className={styles.sheet} onClick={(event) => event.stopPropagation()}><span>{record.data.date}</span><h2>{record.data.events.length > 0 ? '루틴을 지킨 날이에요' : '기록을 기다리는 날이에요'}</h2><DataStateNotice dataState={record.data.dataState} lastSyncedAt={record.data.lastSyncedAt}/><p>{record.data.events[0]?.title ?? '내일의 기록을 기다리고 있어요.'}</p><button className={styles.secondary} onClick={onClose}>닫기</button></section></div>
}

function RecordJourney() { const records = useRecords(); const [selectedDate, setSelectedDate] = useState<string | null>(null); const navigate = useNavigate(); const waiting = QueryState({ loading: records.isLoading, error: records.error, retry: () => void records.refetch() }); if (waiting) return waiting; if (!records.data) return null; return <MobileShell><section className={styles.page}><p className={styles.eyebrow}>30일 기록 여정</p><h1>하루씩 남긴 변화</h1><DataStateNotice dataState={records.data.dataState} lastSyncedAt={records.data.lastSyncedAt}/><p>각 날짜를 눌러 MyData 반영과 루틴 기록을 확인해요.</p><div className={styles.calendar}>{records.data.items.map((item) => <button aria-label={`${item.date} 기록`} className={item.events.length > 0 ? styles.dayDone : styles.day} key={item.date} onClick={() => setSelectedDate(item.date)}>{Number(item.date.slice(-2))}</button>)}</div><button className={styles.primary} onClick={() => navigate('/demo')}>데모 진행</button></section>{selectedDate && <RecordDaySheet date={selectedDate} onClose={() => setSelectedDate(null)}/>}<Tabs/></MobileShell> }
...
function DemoAdvance() {
  ...
  const completeDemo = async () => {
    ...
    if (!goalId) return
    let expectedStage = getDemoExpectedStage(goalId)
    try {
      while (expectedStage < 3) {
        expectedStage = (await advance.mutateAsync(expectedStage)).stage
        saveDemoExpectedStage(goalId, expectedStage)
      }
