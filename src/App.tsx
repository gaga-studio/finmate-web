...
import { AppShell, BottomNav, IconBadge, StatusBar } from './design-v2/primitives'
import { AppSectionCard, BottomSheet, ConsentRow, ScreenLead, SectionHeading } from './design-v2/components'
import { MateAvatar, MatePointPill, MateSectionCard, RpgIcon } from './design-v2/MateShared'
...
function StateView({ state, onRetry }: { state: 'loading' | 'empty' | 'stale' | 'error'; onRetry?: () => void }) {
  ...
  return <div className="screen screen-compare compare-flow-screen state-view"><section className="compare-flow-body"><div
    aria-label={state === 'loading' ? '데이터 불러오는 중' : content[0]}
    className="empty-state-panel app-section-card state-panel"
    role={state === 'error' ? 'alert' : 'status'}
  >
    {state === 'loading'
      ? <span className="state-spinner" aria-hidden="true"><i/><b>✨</b></span>
      : <span className="state-icon-wrap"><IconBadge icon={state === 'error' ? 'shield' : 'spark'} tone={state === 'error' ? 'warning' : 'teal'}/></span>}
    <strong>{content[0]}</strong>
    <p>{content[1]}</p>
    {onRetry && <button className="app-button secondary" onClick={onRetry}>다시 시도</button>}
  </div></section></div>
}
...
function GoalSuccess() { const navigate = useNavigate(); return <MobileShell><div className="screen onboarding-screen"><StatusBar time="9:41"/><div className="onboarding-shell"><div className="onboarding-content"><AppSectionCard className="completion-card"><MateAvatar species="me" size={132} fit="contain"/><SectionHeading eyebrow="목표가 시작됐어요" title="여행까지 한 걸음" subtitle="목표 금액과 현재 저축 흐름을 바탕으로 홈 레이드를 준비했어요."/><button className="app-button primary" onClick={() => navigate('/home')}>홈으로 가기</button></AppSectionCard></div></div></div></MobileShell> }
...
function GroupDetail() {
  ...
  return <MobileShell><div className="screen screen-compare screen-tab-main screen-mate-main">{adventurers.data && report.data ? <><MateSectionNav active="groups"/><section className="screen-stack tab-main-stack mate-main-stack"><DataStateNotice dataState={adventurers.data.dataState} lastSyncedAt={adventurers.data.lastSyncedAt}/><GroupInsightView report={report.data}/><MateGroupDetailView group={group} adventurers={adventurers.data.items}/></section></> : null}</div><Tabs/></MobileShell>
}
...
function Quests() {
  ...
  return <MobileShell><div className="screen screen-compare screen-tab-main screen-mate-main screen-quest-main"><MateTopHeader subtitle="오늘의 작은 행동으로 금융 루틴을 이어가요" pointBalance={points.data.balance}/><DataStateNotice dataState={quests.data.dataState} lastSyncedAt={quests.data.lastSyncedAt}/><section className="screen-stack tab-main-stack mate-main-stack quest-main-stack"><QuestOverviewView page={quests.data} pointLedger={points.data} cosmetics={cosmetics.data}/></section></div><Tabs/></MobileShell>
}
...
function RecordJourney() {
  ...
  return <MobileShell><RecordJourneyMap journey={records.data} pointBalance={points.data.balance} onChangeMonth={changeMonth} onSelectDate={setSelectedDate}/><button className="roadmap-demo-link" onClick={() => navigate('/demo')}>시연 시간 진행</button>{selectedDate && <RecordDaySheet date={selectedDate} onClose={() => setSelectedDate(null)}/>}<Tabs/></MobileShell>
}
