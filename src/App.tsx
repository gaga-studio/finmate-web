...
import { useActiveRoutine, useAdvanceDemo, useAdventurerRoutine, useAdventurers, useChooseAdaptation, useCompleteOnboarding, useCompleteQuest, useCreateAdaptation, useDailyRecord, useHome, useImportRoutine, useMateGroups, useMonthlyReport, useQuests, useRecords, useReplaceRoutine, useRaid } from './api/queries'
...
function QueryState({ loading, error, retry }: { loading: boolean; error: unknown; retry: () => void }) { return loading ? <MobileShell><StateView state="loading"/></MobileShell> : error ? <MobileShell><StateView state={errorState(error)} onRetry={retry}/></MobileShell> : null }
...
function GoalConfirmation() {
  const navigate = useNavigate(); const complete = useCompleteOnboarding(); const draft = getOnboardingDraft(); const form = useForm<Schema['UserGoalDraft']>({ defaultValues: draft.mainGoal })
  const confirm = (mainGoal: Schema['UserGoalDraft']) => {
    saveGoalDraft(mainGoal)
    complete.mutate({ displayName: currentSession()?.user.displayName ?? '민지', mainGoal, confirmMainGoal: true }, { onSuccess: () => navigate('/goal/success') })
  }
  ...
  return <MobileShell><section className={styles.page}><p className={styles.eyebrow}>목표 확인</p><h1>{watchedGoal.title || '유럽 여행 자금'}을 위한 준비</h1><form className={styles.form} onSubmit={form.handleSubmit(confirm)}><label>목표 이름<input aria-label="목표 이름" {...form.register('title')}/></label><label>현재 금액<input aria-label="현재 금액" type="number" {...form.register('currentAmountKrw', { valueAsNumber: true })}/></label><label>목표 금액<input aria-label="목표 금액" type="number" {...form.register('targetAmountKrw', { valueAsNumber: true })}/></label><label>목표 월<input aria-label="목표 월" type="month" {...form.register('targetMonth')}/></label><div className={styles.goalHero}><span>MAIN GOAL</span><strong>{new Intl.NumberFormat('ko-KR').format(watchedGoal.currentAmountKrw || 0)}</strong><small>/ {formatKrw(watchedGoal.targetAmountKrw || 0)}</small><div className={styles.goalTrack}><i style={{ width: `${progress}%` }}/></div><p>현재 MyData 기반으로 확인된 저축 금액이에요.</p></div><p>메이트는 이 목표를 바꾸지 않아요. 함께할 루틴만 제안해요.</p>{complete.error && <small role="alert">{errorState(complete.error) === 'stale' ? '데이터가 조금 오래됐어요.' : '목표를 만들지 못했어요.'}</small>}<button className={styles.primary} disabled={complete.isPending} type="submit">{watchedGoal.title || '유럽 여행 자금'} 목표 만들기</button></form></section></MobileShell>
}
...
function HomeScreen() {
  ...
  const { mainGoal, raid, activeRoutineBuild } = home.data; const progress = Math.round(mainGoal.currentAmountKrw / mainGoal.targetAmountKrw * 100); const goalCompleted = mainGoal.state === 'COMPLETED' || raid.stage === 3
  return <MobileShell><section className={styles.page}><header className={styles.topline}><span className={styles.brand}>FinMate</span><button aria-label="동물 리포트" className={styles.iconButton} onClick={() => navigate('/report')}><Heart size={20}/></button></header><p className={styles.muted}>MyData 기준 마지막 동기화: {home.data.lastSyncedAt ?? '확인 중'}</p><h1>{mainGoal.title} 목표를 향해 가고 있어요.</h1><p>{goalCompleted ? '목표 상태: 완료' : '목표 상태: 진행 중'}</p><div className={styles.raid}><span>RAID STAGE {raid.stage}</span><h2>생활비 드래곤</h2><div className={styles.boss}><i style={{ width: `${100 - raid.bossHpBps / 100}%` }}/></div><p>이번 주 저축 루틴으로 보스를 약하게 만들고 있어요.</p></div><div className={styles.goalRow}><div><span>{mainGoal.title}</span><strong>{formatKrw(mainGoal.currentAmountKrw)}</strong></div><span>{progress}%</span></div>{activeRoutineBuild && <div className={styles.routine}><span>현재 루틴</span><h2>현재 루틴: {activeRoutineBuild.steps[0]}</h2><p>{activeRoutineBuild.difficulty} · {activeRoutineBuild.domain}</p></div>}<button className={styles.secondary} onClick={() => navigate('/report')}>동물 리포트 보기</button></section><Tabs/></MobileShell>
}

function AnimalReportScreen() { const report = useMonthlyReport(); const navigate = useNavigate(); const waiting = QueryState({ loading: report.isLoading, error: report.error, retry: () => void report.refetch() }); if (waiting) return waiting; if (!report.data) return null; return <MobileShell><section className={styles.page}><p className={styles.eyebrow}>MONTHLY MYDATA REPORT</p><h1>오늘의 동물 리포트</h1><div className={styles.report}><Heart size={36}/><h2>저축 HP가 안정적으로 자랐어요.</h2><p>저축 지수 {report.data.financialStats.savingBps / 100}% · 퀘스트 완료 {report.data.completedQuestCount}회</p><p>금융 통계는 MyData 계산 결과이며 퀘스트 보상으로 늘어나지 않아요.</p></div><button className={styles.primary} onClick={() => navigate('/mates')}>메이트 루틴 살펴보기</button></section><Tabs/></MobileShell> }
...
function AdventurerDetail() {
  ...
  return <MobileShell><section className={styles.page}><p className={styles.eyebrow}>ANONYMOUS ADVENTURER</p><h1>{routine.data.title}</h1><div className={styles.avatar}>?</div><p>{routine.data.maintainedDays}일 동안 유지한 익명 루틴이에요.</p><blockquote>“{routine.data.description}”</blockquote><Link className={styles.primary} to={`/routine/${groupId}/${adventurerId}/${routineId}`}>루틴을 내 생활에 맞추기</Link></section><Tabs/></MobileShell>
}

function RoutineScreen() {
  const { groupId = '', adventurerId = '', routineId = '' } = useParams(); const navigate = useNavigate(); const [adaptation, setAdaptation] = useState<Schema['RoutineAdaptationSet'] | null>(null); const [candidateId, setCandidateId] = useState<string | null>(null); const [confirming, setConfirming] = useState(false); const create = useCreateAdaptation(); const choose = useChooseAdaptation(); const active = useActiveRoutine(); const importRoutine = useImportRoutine(); const replace = useReplaceRoutine()
  const prepare = () => create.mutate({ groupId, adventurerId, routineId }, { onSuccess: (set) => choose.mutate({ adaptationId: set.adaptationId, body: { domain: 'SAVING' } }, { onSuccess: setAdaptation }) })
...
function Quests() {
  const quests = useQuests(); const complete = useCompleteQuest(); const [reward, setReward] = useState<Schema['QuestCompletion'] | null>(null); const waiting = QueryState({ loading: quests.isLoading, error: quests.error, retry: () => void quests.refetch() }); if (waiting) return waiting; if (!quests.data) return null
  return <MobileShell><section className={styles.page}><p className={styles.eyebrow}>퀘스트</p><h1>오늘의 작은 행동</h1>{reward && <div className={styles.report} role="status"><h2>퀘스트를 완료했어요</h2><p>XP {reward.xpAwarded} · {reward.internalRewardCodes.join(', ') || '내부 꾸미기 보상'}</p></div>}<div className={styles.questList}>{quests.data.items.map((quest) => <article className={styles.quest} key={quest.questId}><BookOpen size={22}/><div><h2>{quest.title}</h2><p>XP {quest.xpReward} · 내부 꾸미기 보상</p></div><button className={styles.iconButton} aria-label={`${quest.title} 완료`} disabled={complete.isPending || quest.status === 'COMPLETED'} onClick={() => complete.mutate(quest.questId, { onSuccess: setReward })}><ShieldCheck/></button></article>)}</div>{complete.error && <small role="alert">{errorState(complete.error) === 'stale' ? '데이터가 조금 오래됐어요.' : '퀘스트를 완료하지 못했어요.'}</small>}<div className={styles.education}><Compass size={22}/><strong>ETF O/X</strong><p>“ETF는 원금 손실이 절대 없다”는 X예요. 위험을 이해하는 교육일 뿐, 상품 추천이 아니에요.</p></div></section><Tabs/></MobileShell>
}
...
function RecordDaySheet({ date, onClose }: { date: string; onClose: () => void }) {
  ...
  return <div className={styles.scrim} onClick={onClose}><section className={styles.sheet} onClick={(event) => event.stopPropagation()}><span>{record.data.date}</span><h2>{record.data.events.length > 0 ? '루틴을 지킨 날이에요' : '기록을 기다리는 날이에요'}</h2><p>{record.data.events[0]?.title ?? '내일의 기록을 기다리고 있어요.'}</p><button className={styles.secondary} onClick={onClose}>닫기</button></section></div>
}

function RecordJourney() { const records = useRecords(); const [selectedDate, setSelectedDate] = useState<string | null>(null); const navigate = useNavigate(); const waiting = QueryState({ loading: records.isLoading, error: records.error, retry: () => void records.refetch() }); if (waiting) return waiting; if (!records.data) return null; return <MobileShell><section className={styles.page}><p className={styles.eyebrow}>30일 기록 여정</p><h1>하루씩 남긴 변화</h1><p>각 날짜를 눌러 MyData 반영과 루틴 기록을 확인해요.</p><div className={styles.calendar}>{records.data.items.map((item) => <button aria-label={`${item.date} 기록`} className={item.events.length > 0 ? styles.dayDone : styles.day} key={item.date} onClick={() => setSelectedDate(item.date)}>{Number(item.date.slice(-2))}</button>)}</div><button className={styles.primary} onClick={() => navigate('/demo')}>데모 진행</button></section>{selectedDate && <RecordDaySheet date={selectedDate} onClose={() => setSelectedDate(null)}/>}<Tabs/></MobileShell> }
...
function DemoAdvance() {
  ...
  const completeDemo = async () => {
    let expectedStage = 0
    while (expectedStage < 3) expectedStage = (await advance.mutateAsync(expectedStage)).stage
